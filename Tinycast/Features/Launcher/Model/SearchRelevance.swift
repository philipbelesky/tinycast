...
enum FuzzyMatch {
    ...
    static func match(_ query: Query, candidate: String) -> Match? {
        ...
        let typed = match(query.natural, in: candidate)
        ...
        for variant in query.reorderings {
            guard let spread = subsequenceScore(variant.characters, candidate) else { continue }
            if spread > (best?.spread ?? Int.min) {
                best = Match(
                    tier: .subsequence, offset: 0, queryLength: variant.characters.count,
                    candidateLength: candidate.count, spread: spread)
            }
        }
        return best
    }

    private static func match(_ query: Variant, in c: String) -> Match? {
        let q = query.text
        let length = c.count
