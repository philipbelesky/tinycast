...
final class VisibilityStore {
    ...
    func allowsHotKey(_ action: HotKeyAction) -> Bool {
        ...
        case .command(let id), .commandAlternate(let id): id.owner == nil ? isKindEnabled(.command) : true
        case .togglePalette, .togglePaletteAlternate, .togglePaletteThird, .quickAction, .customCommand,
